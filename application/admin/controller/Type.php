...
class Type extends Base
{
    ...
    public function info()
    {
        if (Request()->isPost()) {
            ...
            if ($typeId > 0 && is_array($contentLang)) {
                ...
                foreach ($contentLang as $langCode => $fields) {
                    ...
                    model('ContentLang')->saveFields('type', $typeId, (string)$langCode, $fields, 'manual', 1);
                }
            }
            model('Type')->setCache();
        ...
        $content_lang_data = [];
        if (!empty($res['info']['type_id'])) {
            foreach ($content_lang_enabled as $langCode) {
                $content_lang_data[$langCode] = model('ContentLang')->getFields('type', $res['info']['type_id'], $langCode);
            }
        }
     ...
     * 分类的多语言字段随主表单一起提交保存，这里只负责“翻译并填入”，不单独持久化。
     */
    public function contentLangTranslate()
    {
        ...
        $result = mac_content_translate($text, $from, $to, ['content_type' => 'type']);
        if ($result === '') {
